...
Scenario('Нельзя авторизоваться несуществующему пользователю',  ({ loginPage }) => {
  loginPage.login({
      userName: 'user',
      password: 'secret'
  });
  loginPage.expectingError('Epic sadface: Username and password do not match any user in this service');
});

Scenario('Нельзя авторизоваться заблокированному пользователю',  ({ loginPage, config  }) => {
  loginPage.login({
      userName: 'locked_out_user',
      password: config.credentials.user.password
  });
  loginPage.expectingError('Epic sadface: Sorry, this user has been locked out.');
});
